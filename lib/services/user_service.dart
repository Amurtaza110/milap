...
class UserService {
  ...
  /// Fetch a single user profile from Firestore once
  ...
  /// Stream users for social feed in real-time.
  /// Note: Firestore does not support complex sorting by "preferredCity" server-side
  /// without additional indexes/fields; we keep the same client-side prioritization.
  Stream<List<UserProfile>> streamSocialFeed({String? preferredCity, String? excludeUid}) {
    return _db
        .collection('users')
        .where('isDeactivated', isEqualTo: false)
        .snapshots()
  ...
  /// Stream only Milap+ (Gold) users for the Milap+ Feed.
  Stream<List<UserProfile>> streamMilapPlusFeed({String? preferredCity, String? excludeUid}) {
    return _db
        .collection('users')
        .where('isDeactivated', isEqualTo: false)
        .where('isMilapGold', isEqualTo: true)
        .snapshots()
        .map((snapshot) {
      List<UserProfile> allUsers = snapshot.docs
          .map((doc) => UserProfile.fromMap(doc.data()))
